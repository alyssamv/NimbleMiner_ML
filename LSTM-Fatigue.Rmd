...
category = "Fatigue" # remaining consistent with prior development
```
...
test_notes <- clinical_notes_raw_data %>% 
  ...
  mutate(Label = as.logical(fatigue))
...
labeled_data_path <- file.path(params$data_folder, 'train_fatigue.csv')
...
all_notes = test_notes %>%
  select(note_processed, fatigue, report_no, Label) %>%
...
predict_probs = model_lstm %>% 
  # predict probability of presence of fatigue symptoms
...
data.frame(N.True = n.true, # 48
           N.Predicted = n.predicted, # 102
           Accuracy = cm$overall[["Accuracy"]], # 0.995
           Precision = cm$byClass[["Precision"]], # 0.382
           Recall = cm$byClass[["Recall"]], # 0.812
           F1 = cm$byClass[["F1"]]) # 0.52
```

```{r document_accuracy_w2v}

t = data.frame(report_no = test_notes$report_no,
               truth = test_notes$fatigue,
...
data.frame(N.True = sum(t$truth), # 32
           N.Predicted = sum(t$prediction), # 67
           Accuracy = cm_doc$overall[["Accuracy"]], # 0.727
           Precision = cm_doc$byClass[["Precision"]], # 0.447
           Recall = cm_doc$byClass[["Recall"]], # 0.937
           F1 = cm_doc$byClass[["F1"]]) # 0.606
```

32 out of 143 (22\%) of test notes have evidence of fatigue. When predicting first at the sentence level, and then referencing back to the full document, we achieve an F-score of about 0.6.
...
<!-- dysp <- word_vectors["fatigue", , drop = FALSE] -->
<!-- # find related terms -->
...
data.frame(N.True = n.true, # 48
           N.Predicted = n.predicted.glove, # 134
           Accuracy = cm_glove$overall[["Accuracy"]], # 0.993
           Precision = cm_glove$byClass[["Precision"]], # 0.276
           Recall = cm_glove$byClass[["Recall"]], # 0.770
           F1 = cm_glove$byClass[["F1"]]) # 0.406
```
...
data.frame(N.True = sum(t_glove$truth), # 32
           N.Predicted = sum(t_glove$prediction), # 77
           Accuracy = cm_doc_glove$overall[["Accuracy"]], # 0.657
           Precision = cm_doc_glove$byClass[["Precision"]], # 0.389
           Recall = cm_doc_glove$byClass[["Recall"]], # 0.937
           F1 = cm_doc_glove$byClass[["F1"]]) # 0.550
```
