...
data.frame(N.True = n.true, # 105
           N.Predicted = n.predicted.w2v, # 207
           Accuracy = cm_w2v$overall[["Accuracy"]], # 0.986
           Precision = cm_w2v$byClass[["Precision"]], # 0.386
           Recall = cm_w2v$byClass[["Recall"]], # 0.761
           F1 = cm_w2v$byClass[["F1"]]) # 0.512
```


Note: Word2vec embedding works better with simple NNET model (f=0.526) than with LSTM. Plus, simple NNET model is faster and smaller CPU load.
...
data.frame(N.True = n.true, # 105
           ...
           Precision = cm_glove$byClass[["Precision"]], # 0.415
           Recall = cm_glove$byClass[["Recall"]], # 0.771
           F1 = cm_glove$byClass[["F1"]]) # 0.540
```

GloVe embedding works better with LSTM model than simple NNET. Also, has better performance than the above model. Similar run time and CPU load.