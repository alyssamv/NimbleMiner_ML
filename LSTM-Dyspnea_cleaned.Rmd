...
data <- pad_sequences(sequences, maxlen = maxlen, padding = "post") # change padding to post
...
data.frame(N.True = n.true, # 105
           N.Predicted = n.predicted, # 407
           Accuracy = cm$overall[["Accuracy"]], # 0.971
           Precision = cm$byClass[["Precision"]], # 0.226
           Recall = cm$byClass[["Recall"]], # 0.876
           F1 = cm$byClass[["F1"]]) # 0.359
```
...
predict_x.glove = model_glove_embedding %>% predict_classes(x_test) # predict classes (presence of dyspnea symptoms)
n.predicted.glove = table(predict_x.glove)[[2]]

cm_glove = caret::confusionMatrix(factor(predict_x.glove[, 1], levels = c(0, 1)), 
                            factor(y_test, levels = c(0, 1)),
                            positive = "1")

data.frame(N.True = n.true, # 105
           N.Predicted = n.predicted.glove, # 455
           Accuracy = cm_glove$overall[["Accuracy"]], # 0.968
           Precision = cm_glove$byClass[["Precision"]], # 0.204
           Recall = cm_glove$byClass[["Recall"]], # 0.885
           F1 = cm_glove$byClass[["F1"]]) # 0.332
```

With an flattened embedding layer, metrics are *slightly* worse than previous model, but there is decent gain in efficiency (time to train model and CPU load).
