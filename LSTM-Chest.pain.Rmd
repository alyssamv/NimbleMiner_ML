...
category = "Chest.pain" # remaining consistent with prior development
```
...
test_notes <- clinical_notes_raw_data %>% 
  ...
  mutate(dyspnea = as.numeric(grepl("Dyspnea", Categories)),
         ...
         fatigue = as.numeric(grepl("Fatigue", Categories)),
  ...
  mutate(Label = as.logical(chest.pain))
...
labeled_data_path <- file.path(params$data_folder, 'train_chestpain.csv')

training_notes_raw <- labeled_data_path %>%
  readr::read_csv(skip_empty_rows = TRUE) %>%
...
first_note_row = which(!is.na(training_notes$report_no))[[1]]

training_notes = training_notes[first_note_row:nrow(training_notes), ] %>%
  mutate(report_no = na.locf(report_no),
         Note = removeNumbers(Note)) %>% ####### remove numbers
  filter(Note != "") %>% ####### remove blank rows
  mutate(note_processed = str_squish(Note)) %>%
  transmute(note_processed,
            chest.pain = as.numeric(Label),
            report_head,
            report_no,
            Label) %>%
  filter(!report_head) %>%
...
all_notes = test_notes %>%
  select(note_processed, chest.pain, report_no, Label) %>%
...
predict_probs = model_lstm %>% 
  # predict probability of presence of chest.pain symptoms
...
data.frame(N.True = n.true, # 81
           N.Predicted = n.predicted, # 101
           Accuracy = cm$overall[["Accuracy"]], # 0.992
           Precision = cm$byClass[["Precision"]], # 0.237
           Recall = cm$byClass[["Recall"]], # 0.296
           F1 = cm$byClass[["F1"]]) # 0.263
```

```{r document_accuracy_w2v}

t = data.frame(report_no = test_notes$report_no,
               truth = test_notes$chest.pain,
...
data.frame(N.True = sum(t$truth), # 40
           N.Predicted = sum(t$prediction), # 65
           Accuracy = cm_doc$overall[["Accuracy"]], # 0.657
           Precision = cm_doc$byClass[["Precision"]], # 0.430
           Recall = cm_doc$byClass[["Recall"]], # 0.7
           F1 = cm_doc$byClass[["F1"]]) # 0.533
```

40 out of 143 (27\%) of test notes have evidence of chest.pain. When predicting first at the sentence level, and then referencing back to the full document, we achieve an F-score of about 0.6.
...
<!-- dysp <- word_vectors["chest.pain", , drop = FALSE] -->
<!-- # find related terms -->
...
data.frame(N.True = n.true, # 81
           N.Predicted = n.predicted.glove, # 79
           Accuracy = cm_glove$overall[["Accuracy"]], # 0.992
           Precision = cm_glove$byClass[["Precision"]], # 0.215
           Recall = cm_glove$byClass[["Recall"]], # 0.209
           F1 = cm_glove$byClass[["F1"]]) # 0.212
```
...
data.frame(N.True = sum(t_glove$truth), # 40
           N.Predicted = sum(t_glove$prediction), # 53
           Accuracy = cm_doc_glove$overall[["Accuracy"]], # 0.671
           Precision = cm_doc_glove$byClass[["Precision"]], # 0.433
           Recall = cm_doc_glove$byClass[["Recall"]], # 0.575
           F1 = cm_doc_glove$byClass[["F1"]]) # 0.494
```
