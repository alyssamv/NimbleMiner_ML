...
```{r create_matrixfn, echo=FALSE, eval = FALSE}
###############################################################################################
...
rm(full_report_label_test, 
   full_testing_notes_data,
   report_head,
   testing_notes_data_with_labels,
   testing_notes_data_without_labels,
   training_notes_data_with_labels,
   training_notes_data_without_labels,
   additional_report_head)
```
...
dtMatrix <- create_matrix(corpus_df$note_processed, 
                                      toLower = TRUE, 
                                      removeNumbers = TRUE, 
...
container <- RTextTools::create_container(dtMatrix, 
                                          labels = as.numeric(as.vector(corpus_df$with_labels)), 
                                          trainSize = 1:trainingSize, 
                                          testSize = (trainingSize + 1):nrow(corpus_df), 
                                          virgin = FALSE)
...
#SVM <- cross_validate(container, nfold = 4, "SVM")
...
predMatrix <- create_matrix(predictionData$note_processed, 
...
# recall1 = caret::recall(pred, factor(truth, levels = c(0, 1)), relevant = "1")
# prec1 = caret::precision(pred, factor(truth, levels = c(0, 1)), relevant = "1")
# f1 = caret::F_meas(pred, factor(truth, levels = c(0, 1)), relevant = "1")
